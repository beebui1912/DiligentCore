...
struct TopLevelASDesc DILIGENT_DERIVE(DeviceObjectAttribs)
    ...
    /// In Linked Multi-GPU mode, this selects the physical GPU node that owns the TLAS backing memory.
    ...
    /// In Linked Multi-GPU mode, set additional bits so that secondary nodes can read the TLAS built by another node
